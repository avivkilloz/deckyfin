...
export const GameLibrary: VFC<Props> = ({ serverAPI }) => {
  ...
  const [view, setView] = useState<'library' | 'settings' | 'add-game' | 'game-detail'>('library');
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [gamesRes, folderRes, infoRes] = await Promise.all([
        serverAPI.callPluginMethod<{}, GameConfig[]>('get_games', {}),
        serverAPI.callPluginMethod<{}, GameFolder[]>('scan_games_folder', {}),
        serverAPI.callPluginMethod<{}, { games_folder: string | null }>('get_games_folder', {}),
      ]);
  ...
  if (view === 'settings') {
    return <SettingsPage serverAPI={serverAPI} gamesFolder={gamesFolder} onBack={loadData} />;
  }
  ...
  if (view === 'add-game') {
    return (
      <AddGameWizard
        serverAPI={serverAPI}
        folders={folders}
        onDone={() => { loadData(); setView('library'); }}
  ...
  if (view === 'game-detail' && selectedGame) {
    return (
      <GameDetail
        serverAPI={serverAPI}
        game={selectedGame}
        onBack={() => { loadData(); setView('library'); }}
      />
    );
  }
